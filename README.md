...
**Author:** TInevimbo Musingadi  aka King Tine (HIT)
...
Here are the key figures generated by the evaluation pipeline:

### 3.1 Cross-Lingual J-Neuron Detection (ROC Curves)
The English-trained J-Neuron probe generalizes across multiple African languages (Afrikaans, Kiswahili, isiZulu, and Shona):
![ROC Curves](results/cross_lingual_roc.png)

### 3.2 Jailbreak Success Rate (ASR) Before & After Circuit Breaker
Activating the token-level circuit breaker reduces the jailbreak success rate to 0% across all evaluated languages:
![ASR Comparison](results/asr_comparison.png)

### 3.3 J-Neuron Causal Activation Scaling Sweep (α-scaling)
Scaling the J-neuron activations dynamically controls the model's compliance with jailbreak instructions:
![Causal Scaling Sweep](results/causal_scaling_sweep.png)
